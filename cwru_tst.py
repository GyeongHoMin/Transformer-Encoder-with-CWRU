...
Dataset: https://www.kaggle.com/datasets/astrollama/cwru-case-western-reserve-university-dataset
"""
...
# ============================================================
# Kaggle에서 CWRU 데이터셋을 받아 ./data/cwru/ 에 .mat 파일들을 넣으세요
DATA_PATH = "./data/cwru"
...
def get_de_key(mat):
    """.mat 파일에서 Drive End 진동 신호 키 탐색 (파일마다 X097, X118 등 번호 다름)"""
    for k in mat.keys():
        if 'DE_time' in k:
            return k
    return None
...
def load_data(path):
    """모든 .mat 파일을 슬라이딩 윈도우로 잘라 (X, y) 반환"""
    ...
    for fname in sorted(os.listdir(path)):
        ...
        signal = mat[de_key].flatten()   # (N, 1) → (N,)
...
def prepare_dataloaders(X, y, batch_size=64):
    """정규화 → 텐서 변환 → Train/Val/Test 분할 → DataLoader 생성"""
    # Z-score 정규화 (평균 0, 표준편차 1)
    # 클래스마다 진동 크기가 달라 값의 크기가 아닌 패턴으로 학습하게 함
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    # numpy → PyTorch 텐서, unsqueeze로 (N,1024) → (N,1024,1)
...
# STEP 3. 학습 + 평가
# ============================================================
def train(model, train_loader, val_loader, n_train, n_val, device, epochs=20):
...
def evaluate(model, test_loader, device, save_path="./results/confusion_matrix.png"):
    model.eval()
...
if __name__ == "__main__":
    ...
    print(f"X shape: {X.shape}, 클래스 분포: {np.bincount(y)}")
    ...
    model = TST().to(device)
    print(model)
    train(model, train_loader, val_loader, n_train, n_val, device, epochs=20)

    # 모델 저장
    os.makedirs("./results", exist_ok=True)
    torch.save(model.state_dict(), "./results/tst_model.pth")
